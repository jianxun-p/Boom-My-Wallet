...
const DISPLAY_ACOUNT_INFO_WHITELIST = ['uid'];
...
function getApiKeys(user: User): Promise<{ name: string; createdOn: Date }[]> {
  ...
  return requestUserApi(user, UserAPI.ListApiKeys)
    .then((data) =>
      (data.apikeys as ApiKey[]).map((k: ApiKey) => {
...
export default function Settings() {
  ...
  const refreshApiKeys = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const apikeys = user.apikeys ?? (await getApiKeys(user));
      setApiKeys(apikeys);
      setUser({ apikeys });
    } finally {
      setLoading(false);
  ...
  const $settings = (
    <div className="settings-wrapper">
      ...
      <section>
        ...
        <div className="settings-content">
          ...
          {settings.map((setting) => (
            <Entry key={setting.name} name={setting.name} value={setting.value} />
          ))}
        </div>
      </section>
      <section>
        <div className="settings-section-header">
          ...
          <button className="settings-add-button" onClick={createApiKey}>
            Add
          </button>
        </div>
        {lastCreatedKey ? (
          <div className="settings-feedback">New API key (copy now): {lastCreatedKey}</div>
        ) : null}
        ...
        <div className="settings-content">
          ...
          {apiKeys.map((k) => (
            <div
              key={`${k.name}-${k.createdOn.getTime()}`}
              onContextMenu={(event) => onApiKeyRightClick(event, k.name)}
            >
      ...
      {contextMenu ? (
        ...
        >
          <button
            className="settings-context-menu-delete"
            onClick={() => deleteApiKey(contextMenu.name)}
          >
      ...
      {contextMenu ? (
        <div className="settings-context-menu-backdrop" onClick={() => setContextMenu(null)} />
      ) : null}
    </div>
  );
